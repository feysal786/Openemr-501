<?php /* Smarty version 2.6.31, created on 2019-04-24 16:23:24
         compiled from default/views/day/ajax_template.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/day/ajax_template.html', 10, false),array('function', 'pc_sort_events', 'default/views/day/ajax_template.html', 390, false),array('modifier', 'date_format', 'default/views/day/ajax_template.html', 385, false),array('modifier', 'string_format', 'default/views/day/ajax_template.html', 386, false),)), $this); ?>
...
$hour = date("H:i");
	...
	$starttimemx = date("i",$rounded);
	$starttimehx = date("H",$rounded);
echo "   <a href='#' title='" . xl("New Appointment") . "' onclick='newEvt(1, $starttimehx , $starttimemx, $Date, 0, 0)' class='css_button'>
...
if (count($facilities) > 1) {
    ...
    if (!$GLOBALS['restrict_user_facility']) echo "    <option value='0' $selected>" . xl('All Clinics') . "</option>\n";
 ...
 // ==============================
 echo "</div>";
 echo "   <select multiple size='18' name='pc_username[]' id='pc_username' class='view2'>\n";
 echo "    <option value='__PC_ALL__'>"  .xl ("Select All"). "</option>\n";
...
foreach ($providers as $provider) {
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        foreach ($events as $event) {
            ...
            $color=$row['color'];
            //feysal remove Clinic Name form popup
			//$divTitle .= "\n" .htmlspecialchars($row['name'],ENT_QUOTES);
			$divTitle .= "\n" .xl('APP Time') . ": ".$dispstarth.": ".$startm."\n";
