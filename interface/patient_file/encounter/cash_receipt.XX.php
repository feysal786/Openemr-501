...
<body onLoad="submitfun()" bgcolor="#ffffff" topmargin=0 rightmargin=0 leftmargin=2 bottommargin=0 marginwidth=2 marginheight=0  >
<p><center>
<?php
 if (sizeof($_GET) > 0) {
  $ar = $_GET;
 ...
 $practice_logo = "../../../custom/login_logo.gif";
 if (file_exists($practice_logo)) {
  echo "<img src='$practice_logo' >\n";
 }
?><br>
...
<a href="javascript:window.close();"><font class=title><?php print $titleres{"lname"} . " " . $titleres{"fname"};?></font></a><br>

<table>
<tr><td><?php xl('Generated on','e'); ?>:</td><td> <?php print oeFormatShortDate(date("d-m-Y"));?></td></tr>
<?php
...
$encounter . "' and pid='$pid'"))
{
	...
	$raw_encounter_date = date("d-m-Y", strtotime($date_result{"date"}));
...
<br><br>
<center><input type="hidden" onclick="printDiv('printableArea')" value="Print" id='Tprint' />
...
function submitfun() {
    document.getElementById("Tprint").click();
	window.close();
}
</script>
<?php
