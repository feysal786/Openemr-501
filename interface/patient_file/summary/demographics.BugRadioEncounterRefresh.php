...
// feysal Que Top 1/6 pass doctorNo/&userid=6&autosave to demographics Removed with auto funcition left here for refrence
 function newEvt6() {
...
///////Appointment Button Removed with auto funcition left here for refrence 
   function newEvt6a() {
...
?>
        var iam = top.tab_mode ? top.frames.editScripts : window[0];
        iam.location.href = "<?php echo $GLOBALS['webroot']?>/interface/patient_file/encounter/load_form.php?formname=LBFPrescriptionNotes&id=<?php echo $formid; ?>"
    };
...
//if(Count==0){newEvt1();}
 ...
 //parent.left_nav.setEncounter("2019-04-19", "1493", enc);
 <?php 
 //feysal When patient click Auto select encounter display in Select and open in tab
 //$encounter = '1517';
 //$_SESSION['encounter'] = $encounter;
 //$date=date("Y-m-d");
$query_result = sqlQuery("SELECT `encounter` FROM `form_encounter` WHERE `date` = ? and pid=? ", array(date("Y-m-d"), $pid));
$encounter=$query_result['encounter'];
if($encounter){
//echo $encounter ;
 //echo "rrrrrrrrrrrrrrrr";
//print $query_result['date'];
//$query_result['date']=date('Y-m-d', time());
 ?>
 //display selected encounter in status
 parent.left_nav.setEncounter('', '<?php echo attr($encounter); ?>', 'enc');
 encurl = 'encounter/encounter_top.php?set_encounter=' + <?php echo attr($encounter);?> + '&pid=' + <?php echo attr($pid);?>;
 top.restoreSession();
// load in encounter in tab
 parent.left_nav.loadFrame('enc2', 'enc', 'patient_file/' + encurl);
 //parent.left_nav.loadFrame('enc2', 'enc', 'patient_file/summary/demographics.php' );
...
///////////////////////
// feysal auto doctors buttons 

if (!acl_check('encounters', 'notes_a')) { 
foreach (array('primary') as $instype) {
    $query = "SELECT * FROM users where authorized=1 and facility_id=3 and active=1 order by fname" ;//.
...
?>
            ...
            <a class='css_button_small'
			   onclick="return document.location.href ='../../main/calendar/add_edit_event.php?patientid=<?php echo htmlspecialchars($pid,ENT_QUOTES); ?>&userid=<?php echo $row['id'] ;?>', '_blank';">
...
// feysal remove long list of events and place inside the loop		
//        $widgetButtonLink = "return newEvt".$row['id']."();";
		$widgetButtonLink = "../../main/calendar/add_edit_event.php?patientid=".$pid. "&userid=".$row['id']."&autosave=1&apptStatus=@ ', '_new' ;";
        $widgetButtonClass = "";
        $linkMethod = "html";
