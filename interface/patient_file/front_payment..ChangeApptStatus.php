...
<head>
    ...
    function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept, $encounter = 0, $copay = 0, $patcopay = 0)
    {
        ...
        //feysal remove Insurance fields
	   ...
	   //feysal auto pick consultation form user table upin
	   $userid        = $_GET['userid'];
	   //print $userid;
	   if($userid){
		   // from Walkin
				$trow = sqlQuery("SELECT `upin` FROM `users` WHERE `id`= ?", array($userid));
			}
			else{
		   // from Menu or Billing and Payment button
	           $trow = sqlQuery("SELECT `upin` FROM `users` WHERE `id`= (SELECT `provider_id` FROM `form_encounter` WHERE `encounter`=?)", array($encounter));
			}
       // Pick Cash only for Today
	   if($date==date("Y-m-d")){
				$consultation=$trow['upin'];
			}
			else{
	           $consultation=0;
	   ...
	   echo "  <td class='detail' align='right'><input type='text' name='" . attr($iname) . "'  id='paying_" . attr($var_index) . "' " .
            " value='$consultation" . '' . "' onchange='coloring();calctotal()'  autocomplete='off' " . 
    ...
    <script language="JavaScript">
        ...
        function closeHow(e) {
			//window.close ();
          if (top.tab_mode) {
               // feysal Exit to Demographics
			   // top.activateTabByName('pat', true);
               // top.tabCloseByName(window.name);
			   document.location.href = 	'/isc/interface/patient_file/summary/demographics.php ' ;
            } else {
                if (opener) {
            ...
            else if (document.getElementById('radio_type_of_payment2').checked == true)//Invoice Balance
            {
                for (var i = 0; i < f.elements.length; ++i) {
                    ...
                    if (ename.indexOf('form_upay[0') == 0) {
                        if (elem.value * 1 > 0) {
                            alert("<?php echo addslashes(xl('Invoice Balance cannot be posted. No Visit is created.')) ?>");
                            return false;
                        }
                        break;
                    }
                ...
                <form class="form-inline" method='post'
                            ...
                            <tr id="tr_radio1" ><!-- For radio Insurance -->
                                ...
                                <td class='text' colspan="2">
                                    ...
                                    <input type="radio" name="radio_type_of_payment" id="radio_type_of_payment_self1"
                                           value="cash"
										   checked="checked"
                                           onClick="make_visible_row();make_it_hide_enc_pay();cursor_pointer();"/><?php echo htmlspecialchars(xl('Visit Payment'), ENT_QUOTES); ?>
                                    </label>
                                </td>
                            </tr>
                            <tr id="tr_radio2" style="display:none"><!-- For radio self -->
                                ...
                                <td class='text' colspan="2">
                                    ...
                                    <input type="radio" name="radio_type_of_payment"
                                           id="radio_type_of_payment1" value="copay" 
                    ...
                    <table class="table table-condensed" id="table_display">
                        ...
                        <tr bgcolor="#cccccc" id="tr_head">
                            ...
                            <th class="dehead">
                                <?php echo htmlspecialchars(xl('Visit No.'), ENT_QUOTES) ?>
                            </th>
