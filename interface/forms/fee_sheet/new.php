...
function echoProductLines()
{
    ...
    foreach ($fs->productitems as $lino => $li) {
        ...
        echo "  <td class='billcell'>$strike1" . text($li['code_text']) . "$strike2</td>\n";
/* feysal remove Modefier from Products
        if (modifiers_are_used(true)) {
            echo "  <td class='billcell'>&nbsp;</td>\n";
        }
*/
...
<?php if (modifiers_are_used(true)) { ?>
  <td style="display:none;" class='billcell'><b><?php echo xlt('Modifiers');?></b></td>
<?php } ?>
...
<?php if (justifiers_are_used()) { ?>
  <td style="display:none;" class='billcell' align='center'<?php echo $justifystyle; ?>><b><?php echo xlt('Justify');?></b></td>
<?php } ?>
