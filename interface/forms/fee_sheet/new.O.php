    ...
    foreach ($fs->productitems as $lino => $li) {
        ...
        if ($billed) {
            ...
            if (fees_are_used()) {
                ...
                echo "  <td class='billcell'>$strike1" . text($li['on_hand']) . "$strike2</td>\n";
                echo "</td>\n";
            }
...
if ($GLOBALS['sell_non_drug_products']) {
    ...
    //made by feysal
    //$tres=sqlStatement("SELECT dt.drug_id, dt.selector, d.drug_id, d.name, SUM(di.on_hand) as on_hand FROM drug_templates AS dt,drugs AS d, drug_inventory AS di".
   //" WHERE d.drug_id = di.drug_id and d.drug_id = dt.drug_id and di.destroy_date is Null GROUP BY d.name, d.size, d.form ORDER BY d.name");
   //made by Saif
   $tres=sqlStatement("SELECT dt.drug_id, dt.selector, d.drug_id, d.name, SUM(di.on_hand) as on_hand FROM drug_templates AS dt,drugs AS d, drug_inventory AS di".
    " WHERE d.drug_id = di.drug_id and d.drug_id = dt.drug_id and di.destroy_date is Null GROUP BY d.name, d.size, d.form, dt.selector ORDER BY d.name");
    ...
    //made by feysal
   	 echo " (".text($trow['on_hand']);
		if ($trow['on_hand'] == '0') echo ' Inventory';   
		echo ") ";
        echo text($trow['name']);
