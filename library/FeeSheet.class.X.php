...
class FeeSheet
{
  ...
  //
    public function addProductLineItem($args)
    {
        ...
        $ddrow = sqlQuery("SELECT on_hand FROM drug_inventory WHERE drug_id = ? AND destroy_date is Null ", array($drug_id) );
    ...
    public function checkInventory(&$prod)
    {
        ...
        if (is_array($prod)) {
            foreach ($prod as $iter) {
                ...
                if ($sale_id) {
                    $query = "SELECT ds.quantity, ds.inventory_id, di.on_hand,di.drug_id, di.warehouse_id " .
