...
class FeeSheet
{
  ...
  //
    public function addProductLineItem($args)
    {
        ...
        $ddrow = sqlQuery("SELECT sum(on_hand) as on_hand FROM drug_inventory WHERE drug_id = ? AND destroy_date is Null ", array($drug_id) );
	
		$on_hand= $ddrow['on_hand'];
		$li['on_hand']=$on_hand;
    ...
    public function checkInventory(&$prod)
    {
        ...
        if (is_array($prod)) {
            foreach ($prod as $iter) {
                ...
                if ($sale_id) {
                    $query = "SELECT ds.quantity, ds.inventory_id,di.drug_id, di.on_hand, di.warehouse_id " .
					...
					$query2="SELECT d.drug_id, d.name, SUM(di.on_hand) as on_hand FROM drugs AS d, drug_inventory AS di".
					" WHERE d.drug_id = di.drug_id and di.drug_id = ".$dirow['drug_id']." and di.destroy_date is Null GROUP BY d.name, d.size, d.form ORDER BY d.name";
					
					$dirow2= sqlQuery($query2);
                    ...
                    if (!empty($dirow['inventory_id'])) {
                        if ($warehouse_id && $warehouse_id != $dirow['warehouse_id']) {
                            ...
                            if (($dirow2['on_hand'] + $dirow['quantity'] - $units) < 0) {
                                $insufficient = $drug_id;
                            }
                        }
                    }
        ...
        if ($insufficient) {
            ...
            $alertmsg = xl('Insufficient inventory for product') . ' "' . $drow['name'] . '".' . ' "' . $dirow2['on_hand'] . '".'. ' "' . $dirow['drug_id'] . '".';
            if ($expiredlots) {
