...
//     }
	...
	function multiprint_header(& $pdf, $p) {
		...
		//print header
		//$pdf->ezImage($GLOBALS['oer_config']['prescriptions']['logo'],'','95','','right','');
		$pdf->ezImage('./interface/pic/qih.jpg','','550','','left');
		$pdf->ezColumnsStart(array('num'=>2, 'gap'=>10));
		//$pdf->ezImage($GLOBALS['oer_config']['prescriptions']['logo'],'','95','','left','');
		$res = sqlQuery("SELECT concat('<b>',f.name,'</b>\n',f.street,'\n',f.city,', ',f.state,' ',f.postal_code,'\nTel:',f.phone,if(f.fax != '',concat('\nFax: ',f.fax),'')) addr FROM users JOIN facility AS f ON f.name = users.facility where users.id ='" .
			add_escape_custom($p->provider->id) . "'");
		$pdf->ezText($res['addr'],12);
		$pdf->ezText('<b>' . xl('Dr. Sampe Ali (Medical Officer). MBBS, RMP') . '</b>',12 );
		//$pdf->ezText( xl('                             MBBS, RMP') ,12 );
		$my_y = $pdf->y;
		$pdf->ezNewPage();
		//$pdf->ezImage($GLOBALS['oer_config']['prescriptions']['qih.jpg'],'','95','','left','');
		
		//$pdf->ezText('<b>' . $p->provider->get_name_display() . '</b>',12 , array('justification'=>'right') );
		$pdf->ezText('<b>' . xl('Dr. Demo Zia Sample') . '</b>',12 , array('justification'=>'right') );
		$pdf->ezText('<b>' . xl('MBBS, MSc, FCPS') . '</b>',12 , array('justification'=>'right') );
		$pdf->ezText('<b>' . xl('Consultant Clinical Oncologist') . '</b>',12 , array('justification'=>'right') );
		$pdf->ezText('<b>' . xl('Head Oncology Department') . '</b>',12 , array('justification'=>'right') );
		// Vertical line
		$pdf->Line(375,100,375,640);
	...
	if ($this->is_faxing || $GLOBALS['rx_show_NPI']) {
            $pdf->ezText('<b>' . '</b>' . $p->provider->npi, 12, array('justification'=>'right'));
        }
	else {
    ...
    if ($GLOBALS['rx_enable_SLN']) {
		...
		$res = sqlQuery("select weight,bps,bpd,temperature,height,pulse,oxygen_saturation,BMI,date,DATE_FORMAT(date, '%Y-%m-%d') from form_vitals where pid =". add_escape_custom($p->patient->id) ." AND DATE_FORMAT(date, '%Y-%m-%d') = CURDATE() AND id = (select max(id) from form_vitals where pid =". add_escape_custom($p->patient->id).")");
		//$pdf->ezText( xl('T=').date('d-m-Y'),10);
		//print date();
		if ($res['date']) {
			$pdf->ezText('<b>' . xl('Vitals') . '</b>',10);
			$pdf->ezText( xl('T=').round($res['temperature'],0).xl('F BP=').$res['bps'].xl('/').$res['bpd'].xl(' Pulse=').round($res['pulse'],0).xl('/min').xl(' SPO2=').round($res['oxygen_saturation'],0).xl('%'),11 );
			$res['weight']= round($res['weight']/2.2, 1);
			$res['height']=$res['height']*2.54;
			$pdf->ezText( xl('Weight=').round($res['weight'],0).xl('kg').xl(' Height=').round($res['height'],0).xl('cm  BSA=').$res['BMI'].xl('').xl('kg/m^2'),11 );
			//$pdf->ezText('');
			}
		//$pdf->line($pdf->ez['leftMargin'],$pdf->y,$pdf->ez['pageWidth']-$pdf->ez['rightMargin'],$pdf->y);
		...
		$pdf->ezText('');
		$pdf->line($pdf->ez['leftMargin'],$pdf->y,$pdf->ez['pageWidth']-$pdf->ez['rightMargin'],$pdf->y);
		$pdf->ezText('<b>' . xl('Rx') . '</b>',26);
		//$res = sqlQuery("select content from form_CAMOS where pid =". add_escape_custom($p->patient->id) ." AND id = (select max(id) from form_camos where pid =". add_escape_custom($p->patient->id).")");
		//$pdf->ezText( xl('Advise:') ,12 ,array('aleft'=>380));
		//$pdf->ezText($res['content'],12 ,array('aleft'=>380) );
		//$pdf->ezText( xl('Advise:') ,12 ,array('aleft'=>380));
		$pdf->ezText('',10);
	}
	...
	function multiprint_footer(& $pdf, $p) {
		...
		if($GLOBALS['print_next_appointment_on_ledger'] == 1) {
                    ...
                    $events = fetchNextXAppts($current_date2,$p->patient->get_pubpid());
		...
		//$pdf->ezText( '<b>'.xl('Next Appointment').'</b>' . ' : ' . text($next_appoint_date) . ' ' . xlt('Time:') . ' ' . text($next_appoint_time). ' ' . xlt('Hrs') ,11 ,array('aleft'=>380) ); 
					}
					else
					{
		//$pdf->ezText( '<b>'.xl('Next Appointment').'</b>' . ': ' ,11 ,array('aleft'=>380) ); 
					}
    
                   
          } // end ($GLOBALS['print_next_appointment_on_ledger'] == 1)
		//$pdf->ezText( '<b>'.xl('Next Apointment :').'</b>' ,12 ,array('aleft'=>380));
	...
	function get_prescription_body_text($p) {
		$body = '<b>'. text($p->get_drug()) . ' ' . text($p->get_size()) . ' ' . text($p->get_unit_display());
		if ($p->get_form()) $body .= ' [' . text($p->form_array[$p->get_form()]) . "]";
		$body .= "</b>     <i>" .
			text($p->substitute_array[$p->get_substitute()]) . "</i>" .
			'<b>' . xlt('Dispense') . ':</b> <u>' . text($p->get_quantity()) . "</u>\n" .
			'<b>' . '</b> ' . text($p->get_dosage()) . ' ' . text($p->form_array[$p->get_form()]) . ' ' .
	...
	function multiprint_action($id = "") {
		...
		foreach ($ids as $id) {
			...
			if ($on_this_page == 0) {
				...
				$this->multiprint_footer($pdf, $p);
				$pdf->ezNewPage();
//feysal Advice Notes to PDF Prescription usind Textarea from LBF  form_name='Prescription Notes' and  field_id='Text_test'
				$res = sqlQuery("SELECT field_value FROM `lbf_data` WHERE form_id=(select max(form_id) from forms where pid=". add_escape_custom($p->patient->id) ." and form_name='Prescription Notes')and field_id='Text_test'");
				$pdf->ezText( xl('').$res['field_value'],12 ,array('aleft'=>380) );
//feysal				
				$pdf->ezText('',10);
				$pdf->ezText('',10);
				$pdf->ezText('',10);
			...
			if (++$on_this_page > 11 || $p->provider->id != $this->providerid) {
				$this->multiprint_footer($pdf,$p);
				$pdf->ezNewPage();
				$this->multiprint_header($pdf, $p);
				// $print_header = false;
